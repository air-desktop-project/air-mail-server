...
use ams_guard::{Source, Thresholds, Verdict};
use ams_loop_tokio::{
    Delivery, DeliveryFailure, Error, Outcome, Service, SharedGuard, Timeouts, serve_connection,
};
...
const PAIR: Source = Source::V4([127, 0, 0, 1]);

fn config(starttls: bool) -> Config<'static> {
...
fn materiel(nom: &str) -> Option<Materiel> {
    let repertoire =
